...
static void rvdbg_dmi_reset_jtag(RVDBGv013_DMI_t *dmi, bool hard_reset)
{
	...
	jtag_dev_write_ir(&jtag_proc, rvdbg_jtag->dp_jd_index, IR_DTMCS);
	...
	jtag_dev_write_ir(&jtag_proc, rvdbg_jtag->dp_jd_index, IR_DMI);
}
...
static const char *idcode2desc(uint32_t idcode)
{
	for(int i = 0; dev_descr[i].idcode; i++)
		if (dev_descr[i].idcode == idcode)
			return dev_descr[i].descr;
	return NULL;
}
void rvdbg013_jtag_dp_handler(uint8_t jd_index, uint32_t j_idcode)
{
	uint32_t dtmcontrol;
	uint8_t version;
	...
	rvdbg_jtag->dmi.idcode = j_idcode;
	rvdbg_jtag->dmi.descr = idcode2desc(j_idcode);
