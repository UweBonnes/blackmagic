	...
	uint8_t argout_len)
{
	...
	if (rvdbg_read_regs(dmi, HART_REG_GPR_BEGIN + 1, dmi->harts[dmi->current_hart].gp_register_backup,
			backup_len) < 0)
		return -1;
	...
	if (rvdbg_write_regs(dmi, HART_REG_GPR_BEGIN + 1,
			dmi->harts[dmi->current_hart].gp_register_backup,
			backup_len) < 0)
		return -1;
...
int rvdbg_dmi_init(RVDBGv013_DMI_t *dmi)
{
	...
	switch (dmi->idle) {
		case 0:
			DEBUG_WARN("no run/test state\n");
			break;
		case 1:
			DEBUG_WARN("leave run/test immediately\n");
			break;
		default:
			DEBUG_WARN("stay %d cycles in run/test\n", dmi->idle - 1);
			break;
	}
