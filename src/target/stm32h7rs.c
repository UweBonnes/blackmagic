...
static bool stm32h7rs_mass_erase(target_s *target);
...
bool stm32h7rs_probe(target_s *target)
{
	...
	if (part_id != ID_STM32H7RS)
		return false;
	/* Update part_id on match */
	target->part_id = part_id;

	target->driver = "STM32H7Rx/Sx";
	...
	stm32h7rs_priv_s *priv_storage = calloc(1, sizeof(*priv_storage));
...
static bool stm32h7rs_mass_erase(target_s *target)
{
	...
	if (!stm32h7rs_wait_erase_bank(target, &timeout, FPEC1_BASE))
		return false;
